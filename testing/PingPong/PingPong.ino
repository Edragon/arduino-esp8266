...
/** Flag used by events to wake up loop task */
volatile bool g_task_wakeup = false;
...
// No HW config required
	...
	// On ESP8266 the USB serial is not available immediately
	while (!Serial)
	{
	...
	// Initialize task flags for ESP8266
	g_task_wakeup = false;
...
void loop()
{
	// ESP8266 doesn't use FreeRTOS semaphores, use simple flag polling
	...
	g_task_wakeup = false; // Reset wake up flag
	...
	while (g_task_event_type != NO_EVENT)
	{
		...
		if ((g_task_event_type & CAD_FIN) == CAD_FIN)
		{
			...
			if (tx_cadResult)
			{
				Serial.printf("CAD returned channel busy after %lldms\n", (long long)duration);
				Radio.Rx(RX_TIMEOUT_VALUE);
			}
			else
			{
				Serial.printf("CAD returned channel free after %lldms\n", (long long)duration);
				if (isMaster)
				{
					digitalWrite(LED_BUILTIN, HIGH);
